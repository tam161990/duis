...
function Footer()
{
    ...
    //Page number
    $this->Cell(0,10, $this->title.', lapa '.$this->PageNo().'/{nb}',0,0,'C');
...
// table 4 cols, no header
function DescriptionTableTrase($data)
{
    //Column widths
    $w=array(30,60,30,60);
    //Data
    ...
    {
        $this->SetFont('Arial','B',8);
        $this->Cell($w[0],4,$row[0],0,0,'L');
        $this->SetFont('Arial','',8);
        $this->Cell($w[1],4,$row[1],0,0,'L');
        $this->SetFont('Arial','B',8);
        $this->Cell($w[2],4,$row[2],0,0,'L');
        $this->SetFont('Arial','',8);
        $this->Cell($w[3],4,$row[3],0,0,'L');
        $this->Ln();
    }
}


// table 2 cols, no header
function DescriptionAsimetricalTable($data)
{
    //Column widths
    $w=array(30,150);
    //Data
    foreach($data as $row)
    {
        $this->SetFont('Arial',$row[0][2],$row[0][3]);
        $this->Cell($w[0],4,$row[0][0],0,0,$row[0][1]);
        $this->SetFont('Arial',$row[1][2],$row[1][3]);
        $this->Cell($w[1],4,$row[1][0],0,0,$row[1][1]);
...
// table 2 cols, no header
function HeaderStyledTable($data)
{
    //Column widths
    $w=array(90,90);
    //Data
...
// table 2 cols, no header
function FooterTablePortrait($data)
{
    //Column widths
    $w=array(60,60,60);
    //Data
...
// table 2 cols, no header
function FooterTablePortraitAsimetrikal($data)
{
    //Column widths
    $w=array(60,30,90);
    //Data
...
// table 2 cols, no header , second column bold
function FooterTablePortrait2dBold($data)
{
    //Column widths
    $w=array(60,60,60);
    //Data
...
function FancyTable($headers,$data, $width, $align)
{
    ...
    {
        $this->Cell($width[$i],7,$header,'TB',0,'C',true);
        $i ++;
    }
    $this->Ln();
    ...
    {
      $i = 0;
      if(isset($row['MERGED'])  && $row['MERGED'] != "")
      {
        $this->SetFont('Arial','B',8);
        $this->Cell(0,6,$row['MERGED'],'B',0);
        $this->Ln();
        $this->SetFont('Arial','',8);
        foreach($headers as $key=>$header)
        {
...
function FancyTableTrase($headers,$data, $width, $align, $alignHeader)
{
    ...
    foreach($headers as $header)
    {
        $this->Cell($width[$i],6,$header,'TB',0,$alignHeader[$i]);
        $i ++;
    }
    $this->Ln();
    //Color and font restoration
