...
if($isAdmin || $isEdUser || $isAuditor )
{
	...
	<table cellpadding="0" cellspacing="0" border="0">
		<tr>
		   ...
		   <td class="menu_cell" ><a  target="frame_1" href="<?=$CalcNormsListLink;?>" onclick="reloadFrame(2,'<?=$editCalcNormsLink;?>');reloadFrame(3,'');enableFrameControl();window.top.min(0);window.top.normal();hideMenu();"><?=text::get('CALC_WORK_H_NORMS');?></a></td>
		   <td class="menu_cell" ><a  target="frame_1" href="<?=$writeOffCodeListLink;?>" onclick="reloadFrame(2,'<?=$editWriteOffCodeLink;?>');reloadFrame(3,'');enableFrameControl();window.top.min(0);window.top.normal();hideMenu();"><?=text::get('WRITE_OFF_CODES');?></a></td>
           <?
if(!$isAuditor)
{
?>
	  ...
	  ?>
		<table cellpadding="0" cellspacing="0" border="0">
			<tr>
				...
				{
				?>
				<td class="menu_cell<?= (!$isAdmin && !$isEditor && !$isProjector)? '_2': '';?>" nowrap><a  target="frame_1" href="<?=$projSearchByNumberLink;?>" onclick="reloadFrame(2,'<?=$actListLink;?>');reloadFrame(3,'');enableFrameControl();window.top.normal();hideMenu();"><?=text::get('SEARCH_BY_NUMBER');?></a></td>
				<?
				}
		...
		<td class="head_table_top">
			...
			<table cellpadding="3" cellspacing="0" border="0">
				<tr>
					...
					<td nowrap id="tdProject" class="menu_cell_top" <?=($isAdmin ||  $isProjector || $projectApprover)?'':'disabled';?>><?=($isAdmin ||  $isProjector ||  $projectApprover)?'<a onclick="subMenu(\'projectSubMenuDiv\',\'tdProject\');return false;" href="#">':'';?><?=text::get('PROJECTS');?><?=($isAdmin ||  $isProjector || $projectApprover)?'</a>':'';?></td>
					...
					<td nowrap id="tdPpaReport" class="menu_cell_top" <?=($isAdmin || $isPpaUser)?'':'disabled';?>><?=($isAdmin || $isPpaUser)?'<a onclick="reloadFrame(23,\'\');disableFrameControl();window.top.min(1);hideMenu();" href="'.$cPpaReportMain.'" target="frame_1">':'';?><?=text::get('PPA_MODULE');?><?=($isAdmin || $isPpaUser)?'</a>':'';?></td>
